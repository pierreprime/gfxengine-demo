...
int loadMask(char *filename,int noImage)
{
    ...
    if (fichier==NULL) {printf("\n Erreur ouverture fichier mask");return -1;}
...
void blitImageBankTra(int ibno,int dx,int dy)
{
	...
	if ((ibno<0) || (ibno>(MAX_IMAGEBANK_TRA-1))) {printf("\n !!!! Dépassement ImageBankTra !!!!: %d",ibno);return;}
	if (imageBankTra[ibno].chargee==0){printf("\n !!!! ImageBankTra non chargee !!!!: %d",ibno);return;}
